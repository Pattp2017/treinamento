(() => {
  ...
  function filtrosAtuais(){return {empresa:norm(document.getElementById('histEmpresa').value),treinamento:norm(document.getElementById('histTreinamento').value),tipo:norm(document.getElementById('histTipo').value),turmaId:document.getElementById('histTurma').value,participante:norm(document.getElementById('histParticipante').value),cpf:cpfNum(document.getElementById('histCpf').value),dataInicial:document.getElementById('histDataInicial').value,dataFinal:document.getElementById('histDataFinal').value};}
  ...
  function montarGrupos(f){return turmas.filter(t=>{const d=String(t.data_inicio||t.criado_em||'').slice(0,10);return (!f.empresa||norm(t.empresa)===f.empresa)&&(!f.treinamento||norm(t.treinamento)===f.treinamento)&&(!f.turmaId||String(t.id)===String(f.turmaId))&&(!f.dataInicial||d>=f.dataInicial)&&(!f.dataFinal||d<=f.dataFinal);}).map(t=>{
  ...
  async function carregarTudo(){const [e,t,tu,p,a,h]=await Promise.all([supabaseFetch('empresas?ativo=eq.true&select=id,nome&order=nome.asc'),supabaseFetch('treinamento_treinamentos?ativo=eq.true&select=id,nome&order=nome.asc'),supabaseFetch('treinamento_turmas?select=id,codigo,agenda_id,empresa,treinamento,data_inicio,criado_em&order=criado_em.desc&limit=2000'),supabaseFetch('treinamento_turma_participantes?select=turma_id,nome,cpf&order=nome.asc&limit=5000'),supabaseFetch('treinamento_agenda?select=id,id_turma,etapa,data_inicio&limit=2000'),supabaseFetch('treinamento_historico_documentos?select=*&order=criado_em.desc&limit=5000')]);empresas=e||[];treinamentos=t||[];turmas=tu||[];participantes=p||[];agendas=a||[];historico=h||[];preencherFiltrosBase();limparResultados();}
  ...
  window.renderHistoricoGithub=async function(){
    ...
    document.getElementById('btnHistLimpar').onclick=()=>{['histEmpresa','histTreinamento','histTipo','histTurma','histParticipante','histCpf','histDataInicial','histDataFinal'].forEach(id=>document.getElementById(id).value='');atualizarFiltrosDependentes();limparResultados();};
